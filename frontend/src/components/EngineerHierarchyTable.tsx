...
export function EngineerHierarchyTable({ workFeedback, poles, isLoading, error, targetRequest }: { workFeedback: WorkFeedback[]; poles: Pole[]; isLoading: boolean; error: string | null; targetRequest?: { pole: Pole; request: number } | null }) {
    ...
    const searchQuery = recordSearch.trim().toLowerCase();
    const isSearching = searchQuery.length > 0;
    const matchingPoles = hierarchyPoles
        .filter((item) => levels.every((level) => !selection[level] || item[level] === selection[level]))
        .filter((item) => !searchQuery || (item.pole.poleId ?? "").toLowerCase().includes(searchQuery) || item.substation.toLowerCase().includes(searchQuery))
    ...
    useEffect(() => {
        ...
        setRecordSearch(item.pole.poleId?.trim() ?? "");
        setRiskFilters([]);
        setLandFilters([]);
    ...
    return <section className="engineer-panel hierarchy-panel" id="pole-risk-records">
        <div className="engineer-heading"><div><span className="hierarchy-kicker">Network hierarchy</span><h3>{isSearching ? "Search results" : currentLevel ? `Select ${levelLabels[currentLevel]}` : `${selection.streetName} Pole Candidates`}</h3><p>{isSearching ? "Matching pole records are shown below. Clear the search to return to the network hierarchy." : "Search by pole ID or substation name to view matching records, or drill down through the network hierarchy."}</p></div><div className="hierarchy-heading-actions"><button className="export-work-orders-button" type="button" onClick={exportHierarchyWorkbook} disabled={matchingPoles.length === 0}>Export Excel</button><span className="data-tag">{matchingPoles.length} records</span></div></div>
        <div className="hierarchy-navigation-row">
            <nav className="hierarchy-breadcrumbs" aria-label="Hierarchy path">
                <button type="button" onClick={() => { setSelection({}); setRecordSearch(""); }}>All records</button>
        ...
        <div className="table-controls hierarchy-controls">
            <label className="pole-search">SEARCH POLE ID OR SUBSTATION<input value={recordSearch} onChange={(event) => setRecordSearch(event.target.value)} placeholder="Enter Pole ID or substation" type="search" /></label>
            ...
            <button className="reset-filters-button" type="button" onClick={() => { setRecordSearch(""); setRiskFilters([]); setLandFilters([]); }} disabled={!recordSearch.trim() && riskFilters.length === 0 && landFilters.length === 0} aria-label="Reset filters" title="Reset filters"><svg aria-hidden="true" viewBox="0 0 24 24" width="18" height="18"><path d="M4 4v6h6M5.6 15a7 7 0 1 0 .7-7.9L4 10" /></svg></button>
        </div>
        ...
        {isLoading ? <p className="loading">Loading hierarchy…</p> : error ? <p className="loading">{error}</p> : matchingPoles.length === 0 && isSearching ? <p className="loading" role="status">No records match your search and current filters.</p> : currentLevel && !isSearching ? <HierarchyLevelTable level={currentLevel} values={values} poles={matchingPoles} onChoose={choose} /> : <CandidateTable rows={matchingPoles} onDetails={setSelectedPole} focusPoleId={targetRequest?.pole.poleId ?? null} />}
